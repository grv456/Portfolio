...
`Elastic Stack` `Splunk SIEM` `Splunk SOAR` `DevSecOps` `Python` `REST APIs` `SPL` `Kibana` `Zabbix` `OpenTelemetry` `Git` `Cloud Security`

## Experience Highlights

- Senior Associate Consultant at Infosys, migrating enterprise monitoring to Elastic Stack and Zabbix.
- Ex-Accenture Software Engineer with 4 years across SIEM/SOAR integrations and security automation.
- Built 10+ SOAR playbooks integrated with EDR, email gateways, and threat intelligence sources.
- Reduced analyst workload by 60% using Python-based enrichment and automation.
- Reduced playbook deployment time by 50% through reusable templates and streamlined approvals.
